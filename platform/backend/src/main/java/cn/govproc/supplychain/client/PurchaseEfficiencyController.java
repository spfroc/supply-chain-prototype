...
public class PurchaseEfficiencyController {
    ...
    @GetMapping("/imports/{taskId}") Map<String,Object> importTask(@PathVariable long taskId){authorization.require("purchase:view");var task=jdbc.sql("SELECT id,file_name AS fileName,status,total_rows AS totalRows,valid_rows AS validRows,invalid_rows AS invalidRows,DATE_FORMAT(created_at,'%Y-%m-%d %H:%i:%s') AS createdAt FROM purchase_import_task WHERE id=:id AND user_id=:userId").params(Map.of("id",taskId,"userId",userId())).query().singleRow();var items=jdbc.sql("""
      SELECT i.id,i.source_row AS rowNumber,i.sku_code AS skuCode,i.quantity,i.sku_id AS skuId,i.address_id AS addressId,i.status,i.error_message AS errorMessage,
        CONCAT_WS(' ',ad.contact_name,ad.contact_phone,ad.province,ad.city,ad.district,ad.detail) AS addressLabel,
      ...
      FROM purchase_import_item i LEFT JOIN product_sku s ON s.id=i.sku_id LEFT JOIN product_spu p ON p.id=s.spu_id LEFT JOIN address ad ON ad.id=i.address_id AND ad.deleted_at IS NULL WHERE i.task_id=:taskId ORDER BY i.source_row
      """).params(Map.of("enterpriseId",enterpriseId(),"taskId",taskId)).query().listOfRows();return Map.of("task",task,"items",items);}
    @PostMapping("/imports/{taskId}/add-to-cart") @Transactional Map<String,Object> addImportToCart(@PathVariable long taskId){authorization.require("purchase:manage");requireImportOwner(taskId);var rows=jdbc.sql("SELECT sku_id AS skuId,quantity,address_id AS addressId FROM purchase_import_item WHERE task_id=:taskId AND status='VALID'").param("taskId",taskId).query().listOfRows();if(rows.isEmpty())throw new IllegalArgumentException("没有可加入购物车的有效商品");int added=0;for(var row:rows){long skuId=((Number)row.get("skuId")).longValue();int quantity=((Number)row.get("quantity")).intValue();Object addressId=row.get("addressId");var existing=jdbc.sql("SELECT id FROM cart_item WHERE user_id=:userId AND sku_id=:skuId AND solution_id IS NULL LIMIT 1").params(Map.of("userId",userId(),"skuId",skuId)).query(Long.class).optional();if(existing.isPresent())jdbc.sql("UPDATE cart_item c JOIN product_sku s ON s.id=c.sku_id SET c.quantity=LEAST(c.quantity+:quantity,s.stock-s.reserved_stock),c.selected=1,c.preferred_address_id=COALESCE(:addressId,c.preferred_address_id) WHERE c.id=:id").param("quantity",quantity).param("addressId",addressId).param("id",existing.get()).update();else jdbc.sql("INSERT INTO cart_item(user_id,sku_id,preferred_address_id,quantity,selected) VALUES(:userId,:skuId,:addressId,:quantity,1)").param("userId",userId()).param("skuId",skuId).param("addressId",addressId).param("quantity",quantity).update();added++;}jdbc.sql("UPDATE purchase_import_task SET status='ADDED' WHERE id=:id").param("id",taskId).update();return Map.of("addedKinds",added);}

    @PutMapping("/imports/{taskId}/address") @Transactional Map<String,Object> assignImportAddress(@PathVariable long taskId,@Valid @RequestBody AddressAssignment request){authorization.require("purchase:manage");requireImportOwner(taskId);int address=jdbc.sql("SELECT COUNT(*) FROM address WHERE id=:id AND enterprise_id=:enterpriseId AND user_id=:userId AND deleted_at IS NULL").param("id",request.addressId()).param("enterpriseId",enterpriseId()).param("userId",userId()).query(Integer.class).single();if(address==0)throw new IllegalArgumentException("配送地址不存在");var statement=jdbc.sql("UPDATE purchase_import_item SET address_id=:addressId WHERE task_id=:taskId"+(request.itemIds()==null||request.itemIds().isEmpty()?"":" AND id IN (:itemIds)")).param("addressId",request.addressId()).param("taskId",taskId);if(request.itemIds()!=null&&!request.itemIds().isEmpty())statement.param("itemIds",request.itemIds());statement.update();return importTask(taskId);}
    ...
    @GetMapping("/imports/{taskId}/quotation.xlsx") void exportQuotation(@PathVariable long taskId,HttpServletResponse response)throws IOException{authorization.require("purchase:view");requireImportOwner(taskId);String enterpriseName=jdbc.sql("SELECT name FROM enterprise WHERE id=:id").param("id",enterpriseId()).query(String.class).single();String generatedAt=java.time.LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));var rows=jdbc.sql("""
      ...
      """).params(Map.of("taskId",taskId,"enterpriseId",enterpriseId())).query().listOfRows();writeWorkbook(response,"采购报价单.xlsx",List.of("采购企业","生成时间","SKU编码","商品名称","数量","市场价","会员价","当前采购价","小计"),rows.stream().map(r->List.of(enterpriseName,generatedAt,r.get("skuCode"),r.get("title"),r.get("quantity"),r.get("marketPrice"),r.get("memberPrice"),r.get("salePrice"),new java.math.BigDecimal(String.valueOf(r.get("salePrice"))).multiply(new java.math.BigDecimal(String.valueOf(r.get("quantity")))))).toList());}
    ...
    record FrequentRequest(@NotNull Long skuId,@Min(1) @Max(9999) int quantity,String remark){}record SelectionRequest(List<Long> skuIds){}record ImportItemRequest(@Min(1) @Max(9999) int quantity){}record AddressAssignment(@NotNull Long addressId,List<Long> itemIds){}
}
