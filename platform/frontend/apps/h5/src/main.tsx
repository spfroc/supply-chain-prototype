...
function H5LoadState({loading,error,empty,emptyText,retry,children}:{loading:boolean;error:string;empty:boolean;emptyText:string;retry:()=>void;children:React.ReactNode}){
  if(loading)return <div className="h5-load-state"><span className="h5-loading-dot"/>正在加载...</div>;
  if(error)return <div className="h5-load-state error"><strong>数据加载失败</strong><small>{error}</small><button onClick={retry}>重新加载</button></div>;
  if(empty)return <div className="h5-load-state empty"><strong>{emptyText}</strong></div>;
  return <>{children}</>;
}
const money = (v: any) =>
...
function H5AfterSales({back}:{back:()=>void}){
  const [rows,setRows]=useState<Row[]>([]);const [eligible,setEligible]=useState<Row[]>([]);const [form,setForm]=useState<Row>();const [error,setError]=useState("");const[loading,setLoading]=useState(true);
  const load=async()=>{setLoading(true);try{setRows(await api<Row[]>("/api/client/service/after-sales"));setError("");}catch(e){setError((e as Error).message);}finally{setLoading(false);}};useEffect(()=>{void load();},[]);
  ...
  return <div className="subpage h5-after-sales"><header><button onClick={back}>‹</button><h1>售后服务</h1><button disabled={loading} onClick={()=>void start()}>申请</button></header><H5LoadState loading={loading} error={error} empty={!rows.length} emptyText="暂无售后申请" retry={()=>void load()}>{rows.map(r=><article className="h5-service-card" key={r.id}><img src={r.image}/><div><strong>{r.title}</strong><small>{r.serviceNo} · {r.orderNo}</small><span>{r.reason}</span></div><em>{status[Number(r.status)]}</em></article>)}</H5LoadState>
