...
function BusinessModule({module}:{module:Module}) {
  ...
  const showMember=(row?:Row)=>{setMemberEditing(row);memberForm.setFieldsValue(row?{...row,password:""}:{roleCode:"BUYER",status:1,password:""});setMemberEditorOpen(true)};
  const saveMember=async()=>{try{const values=await memberForm.validateFields();await business(`/enterprises/${memberEnterprise!.id}/members${memberEditing?`/${memberEditing.id}`:""}`,{method:memberEditing?"PUT":"POST",body:JSON.stringify(values)});setMembers(await business(`/enterprises/${memberEnterprise!.id}/members`));setMemberEditorOpen(false);message.success("企业成员已保存");void rows.refresh()}catch(e){message.error((e as Error).message)}};
  const removeMember=(row:Row)=>modal.confirm({title:`确认删除成员“${row.realName}”？`,okButtonProps:{danger:true},onOk:async()=>{try{await business(`/enterprises/${memberEnterprise!.id}/members/${row.id}`,{method:"DELETE"});setMembers(await business(`/enterprises/${memberEnterprise!.id}/members`));message.success("企业成员已删除");void rows.refresh()}catch(e){message.error((e as Error).message)}}});
  ...
  if(module==="enterprises")columns=[
    ...
    {title:"操作",render:(_,r)=><Space><Button type="link" onClick={()=>void loadMembers(r)}>成员管理</Button><Button type="link" onClick={()=>show(r)}>编辑</Button><Button type="link" danger onClick={()=>remove(r)}>删除</Button></Space>}];
  ...
  return <><Card className="data-card" title={title} extra={module!=="orders"&&<Button type="primary" onClick={()=>show()}>＋ 新增{module==="products"?"商品":module==="enterprises"?"企业":"协议"}</Button>}><Table rowKey="id" loading={rows.loading} dataSource={rows.data} columns={columns} scroll={{x:1000}}/></Card>
    <Modal open={open} title={mode==="stock"?"调整库存":`${editing?"编辑":"新增"}${module==="products"?"商品":module==="enterprises"?"企业":"协议"}`} onCancel={()=>setOpen(false)} onOk={save} width={760}><Form form={form} layout="vertical" className="two-column-form">{mode==="stock"?<Form.Item name="stock" label="总库存" rules={[{required:true}]}><InputNumber min={0} style={{width:"100%"}}/></Form.Item>:module==="products"?<><Form.Item name="title" label="商品标题" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="spec" label="SKU规格"><Input placeholder="例如：黑色 / 标准版"/></Form.Item><Form.Item name="categoryId" label="三级分类" rules={[{required:true,message:"请选择三级分类"}]}><Select options={(categories.data||[]).filter(x=>Number(x.level)===3&&Number(x.status)===1).map(x=>({value:x.id,label:`${x.parentName||""} / ${x.name}`}))}/></Form.Item><Form.Item name="brandId" label="品牌"><Select options={[{value:1,label:"联想"},{value:2,label:"得力"}]}/></Form.Item><Form.Item name="marketPrice" label="市场价" rules={[{required:true}]}><InputNumber min={0} style={{width:"100%"}}/></Form.Item><Form.Item name="memberPrice" label="会员价" rules={[{required:true}]}><InputNumber min={0} style={{width:"100%"}}/></Form.Item><Form.Item name="stock" label="库存" rules={[{required:true}]}><InputNumber min={0} style={{width:"100%"}}/></Form.Item><Form.Item name="status" label="状态"><Select options={[{value:1,label:"在售"},{value:0,label:"草稿"},{value:2,label:"下架"}]}/></Form.Item><Form.Item name="mainImage" label="商品主图" className="full"><Input placeholder="请输入 OSS 图片地址"/></Form.Item><Form.Item name="gallery" label="商品配图" className="full"><Input.TextArea rows={2} placeholder="每行一个图片地址"/></Form.Item><Form.Item name="attributes" label="商品属性" className="full"><Input.TextArea rows={2} placeholder="例如：颜色：黑色；保修：三年"/></Form.Item><Form.Item name="summary" label="商品摘要" className="full"><Input.TextArea rows={2}/></Form.Item><Form.Item name="detailHtml" label="富文本详情" className="full"><Input.TextArea rows={5} placeholder="支持填写商品详细介绍或 HTML 内容"/></Form.Item></>:module==="enterprises"?<><Form.Item name="name" label="企业名称" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="creditCode" label="统一社会信用代码" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="contactName" label="联系人" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="contactPhone" label="联系电话" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="address" label="企业地址" className="full"><Input/></Form.Item><Form.Item name="status" label="状态"><Select options={[{value:1,label:"正常"},{value:0,label:"停用"}]}/></Form.Item></>:<><Form.Item name="enterpriseId" label="签约企业" rules={[{required:true}]}><Select options={(enterprises.data||[]).map(x=>({value:x.id,label:x.name}))}/></Form.Item><Form.Item name="name" label="协议名称" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="amount" label="协议金额" rules={[{required:true}]}><InputNumber min={0} style={{width:"100%"}}/></Form.Item><Form.Item name="status" label="状态"><Select options={[{value:1,label:"生效中"},{value:0,label:"待生效"},{value:2,label:"停用"}]}/></Form.Item><Form.Item name="effectiveDate" label="生效日期" rules={[{required:true}]}><Input type="date"/></Form.Item><Form.Item name="expiryDate" label="到期日期" rules={[{required:true}]}><Input type="date"/></Form.Item></>}</Form></Modal>
    ...
    <Modal open={memberOpen} title={`${memberEnterprise?.name||""} · 企业成员`} width={820} footer={null} onCancel={()=>setMemberOpen(false)}><Button type="primary" style={{marginBottom:16}} onClick={()=>showMember()}>＋ 添加成员</Button><Table rowKey="id" dataSource={members} pagination={false} columns={[{title:"成员",render:(_,r)=><><strong>{r.realName}</strong><small className="subline">@{r.username}</small></>},{title:"手机",dataIndex:"phone"},{title:"角色",dataIndex:"roleCode",render:v=>v==="ENTERPRISE_ADMIN"?"企业管理员":"采购员"},{title:"状态",dataIndex:"status",render:v=><Tag color={Number(v)===1?"green":"default"}>{Number(v)===1?"启用":"停用"}</Tag>},{title:"创建时间",dataIndex:"createdAt",render:dateTime},{title:"操作",render:(_,r)=><Space><Button type="link" onClick={()=>showMember(r)}>编辑</Button><Button type="link" danger onClick={()=>removeMember(r)}>删除</Button></Space>}]}/></Modal>
    <Modal open={memberEditorOpen} title={`${memberEditing?"编辑":"添加"}企业成员`} onCancel={()=>setMemberEditorOpen(false)} onOk={()=>void saveMember()}><Form form={memberForm} layout="vertical"><Form.Item name="username" label="登录账号" rules={[{required:true,message:"请输入登录账号"}]}><Input disabled={!!memberEditing}/></Form.Item><Form.Item name="password" label={memberEditing?"重置密码":"初始密码"} extra={memberEditing?"如不修改密码，请保持为空":undefined} rules={[{required:!memberEditing,message:"请输入初始密码"},{min:8,max:72,message:"密码长度必须为8至72位"}]}><Input.Password autoComplete="new-password" placeholder={memberEditing?"输入新密码（可选）":"请输入8至72位初始密码"}/></Form.Item><Form.Item name="realName" label="姓名" rules={[{required:true,message:"请输入姓名"}]}><Input/></Form.Item><Form.Item name="phone" label="手机号码" rules={[{required:true,message:"请输入手机号码"},{pattern:/^1\d{10}$/,message:"请输入11位手机号码"}]}><Input/></Form.Item><Form.Item name="roleCode" label="企业角色" rules={[{required:true}]}><Select options={[{value:"BUYER",label:"采购员"},{value:"ENTERPRISE_ADMIN",label:"企业管理员"}]}/></Form.Item><Form.Item name="status" label="账号状态"><Select options={[{value:1,label:"启用"},{value:0,label:"停用"}]}/></Form.Item></Form></Modal>
...
function Categories() {
  ...
  const columns:ColumnsType<Row>=[
    {title:"分类名称",render:(_,row)=><div className="user-cell"><i>{row.level}</i><span><strong>{row.name}</strong><small>{row.parentName?`上级：${row.parentName}`:"一级分类"}</small></span></div>},
    {title:"级别",dataIndex:"level",render:value=><Tag color={Number(value)===1?"blue":Number(value)===2?"cyan":"purple"}>{value}级</Tag>},
    {title:"排序",dataIndex:"sortOrder"},{title:"子分类",dataIndex:"childCount",render:value=>`${value} 个`},
    {title:"商品",dataIndex:"productCount",render:value=>`${value} 款`},
    {title:"状态",dataIndex:"status",render:value=><Tag color={Number(value)===1?"green":"default"}>{Number(value)===1?"启用":"停用"}</Tag>},
    {title:"操作",render:(_,row)=><Space><Button type="link" onClick={()=>show(row)}>编辑</Button><Button type="link" danger onClick={()=>remove(row)}>删除</Button></Space>}
  ];
