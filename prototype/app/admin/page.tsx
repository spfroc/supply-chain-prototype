...
const moduleConfigs: Record<string, ModuleConfig> = {
  "商品管理": {
    ...
    rows:[["SPU-202607-0018","联想 ThinkBook 16+ 商务本","办公设备 / 联想","¥6,899.00","86","在售","07-21 15:32"],["SPU-202607-0017","惠普 LaserJet Pro 打印机","办公设备 / 惠普","¥2,499.00","42","在售","07-21 14:18"],["SPU-202607-0016","得力 A4 多功能复印纸","办公耗材 / 得力","¥209.00","328","在售","07-21 11:06"],["SPU-202607-0015","MAXHUB 65英寸会议平板","会议设备 / MAXHUB","¥12,680.00","18","低库存","07-20 17:40"],["SPU-202607-0014","华为 24口千兆交换机","网络设备 / 华为","¥4,299.00","0","已下架","07-20 16:22"]]
  },
  "企业管理": {
    ...
    rows:[["山东高速数字科技","9137************2X","张经理 138****2108","16","2026年度框架协议","已认证","2026-06-18"],["青岛城市建设投资集团","9137************8M","李主任 186****3921","8","办公物资采购协议","已认证","2026-05-26"],["济南轨道交通集团","9137************5F","王经理 185****6642","23","轨交设备采购协议","已认证","2026-04-11"],["山东文旅产业集团","9137************7Q","赵主任 156****8026","1","—","待审核","2026-07-21"],["青岛海洋发展集团","9137************3L","孙经理 137****1203","1","—","待审核","2026-07-20"]]
  },
  "协议管理": {
    ...
    rows:[["AGR-2026-0086","2026年度办公设备采购框架协议","山东高速数字科技","128","¥2,800,000","2026-07-01 ~ 2027-06-30","生效中"],["AGR-2026-0085","办公物资集中采购协议","青岛城市建设投资集团","86","¥1,200,000","2026-06-01 ~ 2027-05-31","生效中"],["AGR-2026-0082","轨交设备采购协议","济南轨道交通集团","56","¥4,600,000","2026-05-01 ~ 2027-04-30","生效中"],["AGR-2026-0078","园区改造物资协议","山东产业园发展集团","42","¥880,000","2026-08-01 ~ 2027-07-31","待生效"],["AGR-2025-0122","2025年度通用物资协议","青岛能源集团","96","¥1,560,000","2025-08-01 ~ 2026-07-31","即将到期"]]
  },
  "订单管理": {
    ...
    rows:[["PO202607210018","山东高速数字科技","6件 / 3地址","¥38,880.00","待确认","待付款","07-21 15:08"],["PO202607210016","青岛城投集团","12件 / 1地址","¥12,460.00","已确认","待发货","07-21 13:42"],["PO202607210011","济南轨道交通","8件 / 2地址","¥86,200.00","已确认","待发货","07-21 10:16"],["PO202607200092","山东港口集团","24件 / 4地址","¥126,800.00","已确认","运输中","07-20 16:40"],["PO202607200086","青岛能源集团","3件 / 1地址","¥8,860.00","已确认","已完成","07-20 11:25"]]
  },
  "方案管理": {
  ...
  "发票管理": {
    ...
    rows:[["INV-202607-0086","PO202607180023","山东高速数字科技有限公司","增值税专用发票","¥28,680.00","07-21 14:20","待处理"],["INV-202607-0085","PO202607170068","青岛城市建设投资集团有限公司","增值税专用发票","¥56,200.00","07-21 11:06","处理中"],["INV-202607-0082","PO202607160042","济南轨道交通集团有限公司","增值税普通发票","¥12,860.00","07-20 15:42","已开具"],["INV-202607-0079","PO202607150012","山东港口集团有限公司","增值税专用发票","¥126,800.00","07-19 10:28","已开具"],["INV-202607-0076","PO202607140086","青岛能源集团有限公司","增值税普通发票","¥8,860.00","07-18 16:52","开具失败"]]
  },
  "内容管理": {
    ...
    rows:[["政企采购平台夏季办公集采活动正式启动","平台公告","运营中心","6,820","2026-07-18","已发布","07-21 14:32"],["企业协议采购下单操作指南","帮助中心","王运营","12,680","2026-07-12","已发布","07-20 16:08"],["办公设备采购常见问题解答","采购指南","李编辑","8,260","2026-07-08","已发布","07-19 11:20"],["关于线下银行转账付款的说明","平台公告","财务中心","9,860","2026-07-01","已发布","07-18 09:42"],["第三季度场景方案推荐专题","营销活动","赵运营","—","—","草稿","07-21 13:16"]]
  }
};
...
function AdminModule({ active, notify }: { active:string; notify:(message:string)=>void }) {
  ...
  return <>
    ...
    {modal&&<div className="moduleOverlay" onMouseDown={()=>setModal(false)}><section className="moduleModal" onMouseDown={event=>event.stopPropagation()}><header><div><span>NEW {config.entity.toUpperCase()}</span><h2>{config.primary}</h2></div><button onClick={()=>setModal(false)}>×</button></header><div className="moduleForm"><label><span>{config.entity}名称 *</span><input placeholder={`请输入${config.entity}名称`} /></label>{active==="协议管理"?<label><span>签约企业 *</span><select defaultValue=""><option value="" disabled>请选择签约企业</option><option>山东高速集团有限公司</option><option>济南城市建设集团有限公司</option><option>山东能源集团有限公司</option><option>青岛海信网络科技股份有限公司</option></select></label>:<label><span>业务编号</span><input placeholder="系统保存后自动生成" disabled /></label>}<label className="wide"><span>说明与备注</span><textarea placeholder="请输入相关说明、业务范围或内部备注" /></label><label><span>生效日期</span><input type="date" /></label><label><span>状态</span><select><option>正常 / 启用</option><option>暂存草稿</option></select></label></div><footer><button onClick={()=>setModal(false)}>取消</button><button className="modulePrimary" onClick={()=>{setModal(false);notify(`${config.entity}已保存`);}}>保存并提交</button></footer></section></div>}
  </>;
}
