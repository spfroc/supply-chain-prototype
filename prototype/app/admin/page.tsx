...
function AdminModule({ active, notify }: { active:string; notify:(message:string)=>void }) {
  ...
  const rows=[...createdRows,...seedRows].filter(item=>item.row.join(" ").includes(keyword));
  const fields=getFormFields(active,tab,enterpriseRecords);
  ...
  const save=async()=>{
    const name=(form.name||form.title||form.platform||"").trim();
    if(!name){notify("请填写必填名称");return;}
    const payload={module:view.module,name,company:form.company,description:form.description||form.detail,effectiveDate:form.effectiveDate,status:form.status,price:Number(form.price)||undefined,data:{...form}};
