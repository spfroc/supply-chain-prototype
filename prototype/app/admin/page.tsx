...
function AdminModule({ active, notify }: { active:string; notify:(message:string)=>void }) {
  ...
  return <>
    ...
    {modal&&<div className="moduleOverlay" onMouseDown={()=>setModal(false)}><section className="moduleModal" onMouseDown={event=>event.stopPropagation()}><header><div><span>NEW {config.entity.toUpperCase()}</span><h2>{config.primary}</h2></div><button onClick={()=>setModal(false)}>×</button></header><div className="moduleForm"><label><span>{config.entity}名称 *</span><input placeholder={`请输入${config.entity}名称`} /></label><label><span>业务编号</span><input placeholder="系统保存后自动生成" disabled /></label><label className="wide"><span>说明与备注</span><textarea placeholder="请输入相关说明、业务范围或内部备注" /></label><label><span>生效日期</span><input type="date" /></label><label><span>状态</span><select><option>正常 / 启用</option><option>暂存草稿</option></select></label></div><footer><button onClick={()=>setModal(false)}>取消</button><button className="modulePrimary" onClick={()=>{setModal(false);notify(`${config.entity}已保存`);}}>保存并提交</button></footer></section></div>}
  </>;
}
