...
function H5Home({ add }: { add: () => void }) {
  return <div className="h5Page"><header className="h5Header"><div><span className="h5Logo">政</span><div><strong>政企采购</strong><small>山东高速数字科技 · 协议已生效</small></div></div><button>♢<i /></button></header><label className="h5Search"><span>⌕</span><input placeholder="搜索商品、品牌、型号..." /><button>搜索</button></label><section className="h5Hero"><span>2026 政企集采季</span><h2>办公采购<br />一站配齐</h2><p>协议专属价格 · 自营正品保障</p><button>立即选购　›</button><b className="h5HeroGoods">💻　🖨️</b></section><div className="h5Shortcuts h5ShopNav">{[["纸","办公耗材"],["文","办公用品"],["电","电脑配件"],["国","国网专区"],["军","军网专区"],["青","青慧采"],["案","定制方案"],["全","全部分类"]].map((item,index) => <button key={item[1]}><i className={`navTone${index}`}>{item[0]}</i><span>{item[1]}</span></button>)}</div><section className="h5Section"><div className="h5SectionHead"><div><span>OFTEN BUY</span><h3>企业常购</h3></div><button>全部 ›</button></div><div className="h5ProductScroll">{quickProducts.map((product) => <H5Product key={product.name} product={product} add={add} />)}</div></section><section className="h5Section"><div className="h5SectionHead"><div><span>SCENE SOLUTION</span><h3>方案推荐</h3></div><button>全部 ›</button></div><div className="h5SolutionStrip"><article><span>智慧办公</span><strong>整体解决方案</strong><small>办公设备 · 网络部署</small></article><article><span>智能会议</span><strong>会议室改造</strong><small>显示 · 音视频系统</small></article></div></section><section className="h5Order"><div className="h5SectionHead"><div><span>HOT PRODUCTS</span><h3>热门商品</h3></div><button>换一批</button></div><div className="h5ProductScroll">{quickProducts.slice().reverse().map((product) => <H5Product key={product.name} product={product} add={add} />)}</div></section></div>;
}
