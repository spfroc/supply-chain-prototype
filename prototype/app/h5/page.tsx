...
function H5Home({ add }: { add: () => void }) {
  return <div className="h5Page"><header className="h5Header"><div><span className="h5Logo">链</span><div><strong>政采供应链</strong><small>协议采购专属平台</small></div></div><button>♢<i /></button></header><label className="h5Search"><span>⌕</span><input placeholder="搜索协议商品、订单" /><button>搜索</button></label><section className="h5Hero"><span><i />企业协议已生效</span><h2>专属采购价<br />为企业降本增效</h2><p>已匹配 128 款协议商品</p><div><strong>本月已节省</strong><b>¥28,640</b></div></section><div className="h5Shortcuts">{[["补","快速补货"],["单","我的订单"],["址","地址管理"],["票","发票管理"]].map((item) => <button key={item[1]}><i>{item[0]}</i><span>{item[1]}</span></button>)}</div><section className="h5Section"><div className="h5SectionHead"><div><span>OFTEN BUY</span><h3>常购清单</h3></div><button>全部 ›</button></div><div className="h5ProductScroll">{quickProducts.map((product) => <H5Product key={product.name} product={product} add={add} />)}</div></section><section className="h5Order"><div className="h5SectionHead"><div><span>TRACKING</span><h3>订单动态</h3></div><button>全部 ›</button></div><div className="h5OrderCard"><div><span className="h5OrderIcon">纸</span><div><strong>办公耗材 · 12项</strong><small>顺丰 SF146829301</small></div><em>运输中</em></div><div className="h5Route"><i className="done" /><b /><i className="done" /><b /><i /><span>预计明日送达</span></div></div></section></div>;
}
